            ...
            <form method="POST" action="{{ route('update-article', $article->id ) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="title" aria-describedby="emailHelp"
                        placeholder="Enter Title" required autocomplete="off" name="title" value="{{ $article->title }}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="url" placeholder="Slug URL" required
                        autocomplete="off" name="url" readonly value="{{ $article->url }}">
                    @error('url')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="input-group mb-3">
                    ...
                    <div class="custom-file">
                        <input type="file" class="custom-file-input" id="inputGroupFile01"
                            aria-describedby="inputGroupFileAddon01" accept="image/*" name="thumbnail" >
                ...
                <div class="form-group">
                    ...
                    <textarea type="text" class="form-control" id="editor" required autocomplete="off" name="editor" readonly>{{ $article->content }}</textarea>
                    @error('content')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
